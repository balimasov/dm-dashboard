...
Проєкт має скіл `prototype` (`.claude/skills/prototype/SKILL.md`) —
фіксує реальні дизайн-токени застосунку (кольори, типографіку, форми
карток/попаверів, іконки) і напрацьовану методологію клікабельного
прототипування. Викликай його через `Skill` ПРОАКТИВНО, без нагадування
від користувача, щоразу перед тим як будувати клікабельний
прототип/мокап (HTML Artifact) під нову фічу, варіанти редизайну чи
будь-яку іншу UI-ідею, яку користувач хоче спершу побачити перед
реальною розробкою.

Так само, як і з `conventions-checker` вище — сам факт існування скіла
не означає, що він автоматично використовується. Тримай цей пункт явно
в увазі щоразу, коли задача звучить як "зроби прототип" / "клікабельний
прототип" / "накидай варіанти дизайну", а не покладайся на пам'ять
розмови.
