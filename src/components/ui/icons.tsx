...
/**
 * Dagger, tapered blade + crossguard + grip — Weapon proficiencies. Built
 * from straight lines only (no freehand curves) — the same monoline stroke
 * convention as `ShieldIcon`/`ToolIcon`/`LanguageIcon` next to it, and
 * scaled from the diagonal's center so tip and pommel reach almost to the
 * edges of the 24×24 grid, the same way `ShieldIcon`'s own silhouette
 * nearly fills its box — a first, smaller-scale pass of this same shape
 * read as visibly thinner/lighter than its neighbors in the row.
 */
export function DaggerIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className={className}>
      <path
        d="M21 3L10.9 16.5M21 3L7.5 13.1M6 11.5L12.5 18M6.7 17.3L4 20"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
