...
/**
 * A bare bracket's inner text (group 3 of `TOKEN_RE`) is either a stray
 * `source_id` the model echoed without ever building the full token (e.g.
 * `feature-0`, `spell_fireball`) or an ordinary display name it
 * over-eagerly bracketed with no id at all (e.g. `Religion`, `Fireball`).
 ...
 * word sits *outside* the bracket and survives regardless). Dropping a bare
 * `[[Religion]]` the same way silently deletes the only occurrence of that
 * word from the reply — this is exactly the bug that lost "Religion" from
 * an otherwise-complete answer. Source ids in this app are always
 * lowercase and either hyphen- or underscore-joined with a trailing
 * word/number (`feature-0`, `spell_fireball`, `trait-2`); an ordinary
 * capitalized word or a phrase with spaces never matches that shape.
 */
 ...
 * `looksLikeSourceId`: an id-shaped one is dropped entirely (the raw
 ...
 * display-name-shaped one is kept as an ordinary `"text"` segment instead —
 * unwrapped, not deleted — so it still gets picked up by the plain-text
 * sheet-term/universal-term scan `AiResponseText` runs on every text
 * segment, same as if the model had never bracketed it at all.
 */
