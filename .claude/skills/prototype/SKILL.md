...
- **Known trap: `#020617`.** That hex is Tailwind's *stock, unmodified* `slate-950` — cool blue-black, not this app's warm reskin. It's a tempting boilerplate default for a ".modal { background: ... }" rule because it looks like a plausible near-black at a glance, but it shipped by copy-paste into two prototypes' `.modal` class this session (both already had the *correct* `--bg` CSS variable defined and even used correctly elsewhere in the same file) and rendered a visibly blue panel the user had to catch from a screenshot. `#020617` does not exist anywhere in this app's real palette — never write it literally; reference `var(--bg)` (or the confirmed `#0f0500`) instead, and double-check every hardcoded background literal in a file before publishing, not just the `:root` variables.
